...
namespace MuscleFellow.Models
{
    public class MuscleFellowSampleDataInitializer
    {
        ...
        {
            // Add Categories
            _dbContext.Set<Category>().Add(new Category { CategoryName = "健身器械" });
            _dbContext.Set<Category>().Add(new Category { CategoryName = "健身补剂" });

            // Add Brands
            _dbContext.Set<Brand>().Add(new Brand
            {
            ...
            tester = "健身器械";
            int equipmentId = _dbContext.Set<Category>().SingleOrDefault(e => e.CategoryName == tester).CategoryID;
            tester = "健身补剂";
            int supplementId = _dbContext.Set<Category>().SingleOrDefault(e => e.CategoryName == tester).CategoryID;

            // Get BrandIds
            tester = "LifeFitness";
            int lifefitnessId = _dbContext.Set<Brand>().SingleOrDefault(b => b.BrandName == tester).BrandID;
            tester = "MASSFIT";
            int massfitId = _dbContext.Set<Brand>().SingleOrDefault(b => b.BrandName == tester).BrandID;
            tester = "MET-Rx(美瑞克斯)";
            int metrxId = _dbContext.Set<Brand>().SingleOrDefault(b => b.BrandName == tester).BrandID;
            tester = "MuscleTech(肌肉科技)";
            int muscletechId = _dbContext.Set<Brand>().SingleOrDefault(b => b.BrandName == tester).BrandID;
            tester = "PROIRON";
            int proironId = _dbContext.Set<Brand>().SingleOrDefault(b => b.BrandName == tester).BrandID;
            ...
            _dbContext.Set<ProductImage>().AddRange(new ProductImage[]
            {
                new ProductImage
                {
                    ProductID = new Guid("5A187D13-3023-4A8B-4448-08D371929CF9"),
        ...
        {
            // Add Set<Province>()
            _dbContext.Set<Province>().AddRange(
                new Province[] {
